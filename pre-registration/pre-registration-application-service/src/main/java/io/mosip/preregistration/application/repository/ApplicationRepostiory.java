...
public interface ApplicationRepostiory extends BaseRepository<ApplicationEntity, String> {
	...
	@Query("SELECT e FROM ApplicationEntity e  WHERE e.crByHash= ?1 order by e.crDtime desc")
	public List<ApplicationEntity> findByCreatedBy(String hashedUserId);
	
	@Query("SELECT e FROM ApplicationEntity e  WHERE e.crByHash= ?1 and e.bookingType= ?2 order by e.crDtime desc")
	public List<ApplicationEntity> findByCreatedByBookingType(String hashedUserId, String bookingType);
